...
import ConditionList from './medication/conditionList';
      ...
      <Card>
        <Stack spacing={3} sx={{ p: 3 }}>
          <ConditionList
            ...
            tableLabels={[
              { id: 'description', label: 'Medication' },
              { id: 'date', label: 'Date Recorded' },
              { id: 'amount', label: 'Assessment Status' },
              { id: 'status', label: 'Average Assessment Score' },
              { id: '' },
            ]}
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const res = await customAxios.post(
        'https://abibiman-api.onrender.com/conditions',
        dataObject,
        {
          headers: {
      ...
      if (res) {
        const {
          data: { items },
        } = await customAxios.get(`https://abibiman-api.onrender.com/conditions/${user.userID}`);
